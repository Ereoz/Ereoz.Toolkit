...
namespace Ereoz.InstallerBase
{
    public static class FilePacking
    {
        ...
        public static void Unpack(string sourceFile, string targetDir, Action<string> onCopiedFileStart, Action onCopiedFileDone)
        {
            using (var fs = File.OpenRead(sourceFile))
            {
                using (var reader = new BinaryReader(fs, Encoding.UTF8))
                {
                    while (fs.Position < fs.Length)
                    {
                        ...
                        onCopiedFileStart?.Invoke(Path.Combine(targetDir, relativePath));
